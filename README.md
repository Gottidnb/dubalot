...
- 🎤 **Preserves the speaker's voice** with a natural accent via XTTS v2 voice cloning
- 🌍 **Translates speech** to any language supported by Google Translate
- 🎵 **Keeps background audio** (music, ambient noise) intact using Demucs source separation
- 👄 **Syncs lips** with the translated audio via Wav2Lip (optional)
- ⏱️ **Aligns timing** by time-stretching each translated segment to match the original
...
Source video
    │
    ├─► [1] Extract audio (ffmpeg / MoviePy)
    │
    ├─► [2] Separate vocals + background (Demucs)
    │
    ├─► [3] Transcribe speech with timestamps (Whisper)
    │
    ├─► [4] Translate segments (Google Translate via deep-translator)
    │
    ├─► [5] Clone voice + synthesise TTS (Coqui XTTS v2)
    │         └─ Time-stretch each segment to original duration
    │
    ├─► [6] Mix translated speech with background audio
    │
    ├─► [7] Apply lip sync (Wav2Lip) ← optional
    │
...
> Requires **Python ≥ 3.8** and **ffmpeg** on your `PATH`.
...
# Provide a custom voice reference clip (3–30 s of clean speech)
...
options:
  ...
  --voice-reference PATH      Custom speaker reference WAV (3–30 s)
...
sounds like that person speaking with a natural accent—satisfying the
...
original utterance.  Rate is clamped to ±2× to avoid excessive distortion.
